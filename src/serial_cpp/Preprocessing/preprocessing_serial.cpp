...
// -----------------------------------------------------------------------------
static std::vector<float> rgb_to_grayscale(
        const uint8_t* rgb,
...
// -----------------------------------------------------------------------------
static std::vector<uint32_t> compute_histogram(
...
// STEP 5 — Binarization
// -----------------------------------------------------------------------------
static std::vector<uint8_t> binarize(
        const std::vector<float>& gray,
        float threshold)
{
    std::vector<uint8_t> binary(gray.size());
...
struct PreprocessResult {
    std::vector<uint8_t> binary;    // Binary image (0 or 255)
    float                threshold; // Computed Otsu threshold
    int                  width;
    int                  height;
        ...
        float sigma = 1.0f)
{
    ...
    auto gray = rgb_to_grayscale(rgb, width, height);
    result.timing.grayscale = elapsed_ms(t0);

    // 2 — Normalization
    t0 = std::chrono::steady_clock::now();
    normalize(gray);
    result.timing.normalize = elapsed_ms(t0);

    // 3 — Gaussian Blur
    t0 = std::chrono::steady_clock::now();
    auto blurred = gaussian_blur(gray, width, height, ksize, sigma);
    result.timing.blur = elapsed_ms(t0);

    // 4a — Histogram
    t0 = std::chrono::steady_clock::now();
    auto hist = compute_histogram(blurred);
    result.timing.histogram = elapsed_ms(t0);

    // 4b — Otsu
    t0 = std::chrono::steady_clock::now();
    result.threshold = otsu_threshold(hist, width * height);
    result.timing.otsu = elapsed_ms(t0);

    // 5 — Binarization
    t0 = std::chrono::steady_clock::now();
    result.binary = binarize(blurred, result.threshold);
    result.timing.binarize = elapsed_ms(t0);

    result.timing.total = elapsed_ms(t_total);
    return result;
}
